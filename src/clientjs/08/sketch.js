...
class Walker {
  ...
  setNewCoords() {
    const generateMoveCoord = (currentCoordValue, mouseCoord) => {
      const targetDirection = mouseCoord < currentCoordValue ? -1 : 1;
      const newDirection = [
        targetDirection,
        0,
        targetDirection,
    ...
    this.newCoords = {
      x: constrain(generateMoveCoord(x, mouseX), 0, width - 1),
      y: constrain(generateMoveCoord(y, mouseY), 0, height - 1),
    };
  }
  setColor() {
    const randomColor = Array.from({ length: 3 }).map(() => generateRandom(random(0, 100)));
